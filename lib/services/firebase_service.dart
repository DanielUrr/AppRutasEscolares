...
class FirebaseService {
  ...
  Future<List<Estudiante>> obtenerEstudiantes() async {
    try {
      QuerySnapshot snapshot = await _firestore
          ...
          .get();
      return snapshot.docs.map((doc) => Estudiante.fromFirestore(doc)).toList();
    } catch (e) {
  ...
  Future<Estudiante?> obtenerEstudiante(String id) async {
    try {
      DocumentSnapshot doc = await _firestore.collection('estudiantes').doc(id).get();
      if (doc.exists) {
        return Estudiante.fromFirestore(doc);
      }
      return null;
    } catch (e) {
  ...
  Future<String> iniciarViaje(Viaje viaje) async {
    try {
      DocumentReference docRef = await _firestore.collection('viajes').add(viaje.toFirestore());
      return docRef.id;
    } catch (e) {
  ...
  Future<void> actualizarUbicacionViaje(String idViaje, LatLng ubicacion) async {
    try {
  ...
  Stream<Viaje?> viajeEnCursoStream(String idConductor) {
    return _firestore
      ...
      return Viaje.fromFirestore(snapshot.docs.first);
    });
  }
  ...
  Stream<List<EventoViaje>> eventosViajeStream(String idViaje) {
    return _firestore
        ...
        .snapshots()
        .map((snapshot) {
      return snapshot.docs.map((doc) => EventoViaje.fromFirestore(doc)).toList();
    });
  }

  Future<List<EventoViaje>> obtenerEventosEstudiante(String idViaje, String idEstudiante) async {
    try {
      QuerySnapshot snapshot = await _firestore
          .collection('viajes')
          .doc(idViaje)
  ...
  Future<List<RetiroManual>> obtenerRetirosDeHoy() async {
    try {
      ...
      QuerySnapshot snapshot = await _firestore
          ...
          .where('fechaRetiro', isGreaterThanOrEqualTo: Timestamp.fromDate(inicioDia))
          .where('fechaRetiro', isLessThan: Timestamp.fromDate(finDia))
          .get();

      return snapshot.docs.map((doc) => RetiroManual.fromFirestore(doc)).toList();
    } catch (e) {
